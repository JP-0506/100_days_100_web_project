...
const GENRES = {
  ...
  "scifi":   { label: "🚀 Sci-Fi & Adventure",
    ids: ["tt0816692","tt1375666","tt0133093","tt2527338","tt4154796",
          "tt0499549","tt1392190","tt0910970","tt1049413","tt2381941"] },
  "action":  { label: "🦸 Action & Superheroes",
    ids: ["tt4154756","tt0848228","tt1825683","tt3501632","tt0800369",
          "tt0468569","tt1345836","tt0458339","tt4154664","tt2395427"] },
  "comedy":  { label: "😄 Comedy & Rom-Com",
    ids: ["tt0298148","tt0454876","tt0268978","tt0381681","tt1655442",
          "tt0120338","tt0386117","tt0435761","tt0317705","tt2024544"] },
  "horror":  { label: "👻 Horror & Thriller",
    ids: ["tt5753856","tt0993842","tt0407887","tt0209144","tt0482571",
          "tt2380307","tt0780504","tt4016934","tt1457767","tt1219289"] },
  "animation":{ label: "🎨 Animation & Family",
    ids: ["tt0910970","tt1049413","tt0435761","tt0317705","tt0266543",
          "tt0382932","tt2096673","tt1727587","tt4116284","tt2948372"] },
  "drama":   { label: "🎭 Drama & Classic",
    ids: ["tt0111161","tt0068646","tt0071562","tt0050083","tt0108052",
          "tt0167260","tt0120737","tt0110912","tt0137523","tt0047478"] },
};
...
movieInput.addEventListener("keypress", e => { if (e.key === "Enter") triggerSearch(); });
...
loadMoreBtn.addEventListener("click", () => { currentPage++; doSearch(false); });
...
async function renderGenreSection(label, ids) {
  ...
  const results = await Promise.all(
    ids.map(id => fetch(`${BASE_URL}?i=${id}&apikey=${API_KEY}`)
      .then(r => r.json()).catch(() => null))
  );

  row.innerHTML = "";

  const validMovies = results.filter(m => m && m.Response !== "False" && hasPoster(m));
  ...
  try {
    ...
    if (data.Response === "False") {
      if (fresh) {
        ...
        moviesContainer.innerHTML = stateMsg("🎭", "No Results Found",
    ...
    validResults.forEach((m, i) => moviesContainer.appendChild(buildCard(m, i)));

    const showing = allMovies.length;
    const yearLabel = year ? ` · ${year}` : "";
    resultsInfo.innerHTML =
      `Showing <span>${showing}</span> of <span>${totalResults}</span> results for "<span>${query}${yearLabel}</span>"`;
    ...
    moviesContainer.innerHTML = stateMsg("⚠️","Something Went Wrong","Check your connection and try again.");
    resultsInfo.innerHTML = "";
  }
}
...
function buildCard(m, i = 0) {
  ...
  const badge = { movie:"🎬 Movie", series:"📺 Series", episode:"🎞️ Ep" }[m.Type] || (m.Type||"");
...
function showSkeletons(n = 10) {
  moviesContainer.innerHTML = Array(n).fill(`
    <div class="skeleton-card">
      <div class="skeleton-poster"></div>
      <div class="skeleton-info">
...
async function getMovieDetails(id) {
  ...
  try {
    const res = await fetch(`${BASE_URL}?i=${encodeURIComponent(id)}&apikey=${API_KEY}&plot=full`);
    const m   = await res.json();
    if (m.Response === "False") throw new Error();

    const poster = hasPoster(m)
      ? m.Poster : "https://placehold.co/260x390/111520/7a8099?text=No+Image";
    ...
    movieDetails.innerHTML = `
      <div class="details-layout">
        ...
        <div class="details-body">
          ...
          ${m.Plot&&m.Plot!=="N/A"?`<div class="plot-section">
